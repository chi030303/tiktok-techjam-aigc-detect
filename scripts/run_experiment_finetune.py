#!/usr/bin/env python3
# 2026-08-30, yun, exp1 entrypoint: partial-unfreeze recipes (freeze_backbone: false)
"""Run one partial-unfreeze experiment from experiments/<name>/recipe.yaml.

Separate from run_experiment.py because src.train.loop.run_train hard-requires
freeze_backbone=true (it extracts features once and only trains the head).
"""
...
def main() -> None:
    ...
    print(f"  partial_unfreeze: {recipe.get('partial_unfreeze')}")
