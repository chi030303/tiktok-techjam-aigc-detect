...
| `requirements.txt` | `.env`, Vast/PSB credentials |
...
4. **Demo / reference only (no train)**  
   COCO val2017 (4998 real) + DALL·E Advanced (8843 fake) → `data/demo_wildfake/`  
   Put a `DO_NOT_TRAIN` file in that folder.
5. **Backbones** (OpenAI CLIP / torchvision ResNet, all ≪ 2B)  
   Pulled by code on first run, or `bash scripts/download_assets.sh --models`

Official inference contract: a directory of images in, JSON out:

```json
[{"image_path": "data/demo_wildfake/0001.jpg", "pred": 0.87}]
```
...
- GPU / Vast / campus cluster: [docs/gpu.md](docs/gpu.md)
