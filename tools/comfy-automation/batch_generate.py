#!/usr/bin/env python3
"""Resumable, bounded-queue PixArt-Sigma batch generator for ComfyUI on Vast."""
...
DEFAULT_NEGATIVE_PROMPT = (
    "malformed hands, deformed hands, poorly formed hands, fused fingers, extra fingers, "
    "missing fingers, duplicated fingers, broken fingers, malformed limbs, extra limbs, "
    "missing limbs, fused limbs, duplicated body parts, disconnected body parts, impossible anatomy, "
    "distorted face, duplicated face, misaligned facial features, malformed eyes, malformed teeth, "
    "melted details, merged objects, fused objects, floating objects, disconnected objects, "
    "impossible geometry, inconsistent perspective, warped straight lines, broken object structure, "
    "incoherent background, repeated objects, cloned elements, repetitive texture, tiled pattern, "
    "waxy skin, plastic skin, overly smooth skin, artificial skin texture, excessive sharpening, "
    "oversharpening halos, color banding, posterization, chromatic artifacts, compression artifacts, "
    "unreadable text, garbled text, random letters, caption, subtitle, watermark, logo, signature, border"
)
...
class WorkflowSpec:
    ...
    def inspect(cls, path: Path) -> "WorkflowSpec":
        ...
        if len(samplers) != 1: raise ValidationError(f"Expected one PixArt KSampler, found {samplers}")
        ...
        dimensions = [str(k) for k, v in workflow.items() if v["class_type"] == "EmptyLatentImage"
                      and isinstance(v["inputs"].get("width"), int) and isinstance(v["inputs"].get("height"), int)]
        ...
        model_loader = linked_node(workflow[samplers[0]]["inputs"].get("model"))
        ...
        if not model_loader or model_loader not in workflow or workflow[model_loader]["class_type"] != "UNETLoader":
            raise ValidationError("PixArt sampler model is not connected to UNETLoader")
        clip_nodes = {linked_node(workflow[n]["inputs"].get("clip")) for n, _ in positives + negatives} - {None}
        if len(clip_nodes) != 1:
            raise ValidationError(f"Expected one shared PixArt CLIPLoader, found {sorted(clip_nodes)}")
        clip_node = next(iter(clip_nodes))
        if (clip_node not in workflow or workflow[clip_node]["class_type"] != "CLIPLoader"
                or workflow[clip_node]["inputs"].get("type") != "pixart"):
            raise ValidationError("Prompt encoders are not connected to CLIPLoader(type=pixart)")
        return cls(workflow, positives, negatives, seeds, saves[0], dimensions[0], tuple(samplers),
                   samplers[0], decode_node, model_loader,
                   frozenset(v["class_type"] for v in workflow.values()))

    def render(self, job: Job, run_name: str | None, negative: str, preset: str) -> dict[str, Any]:
        ...
        if preset == "quality":
            workflow[self.primary_sampler]["inputs"].update(
    ...
    def summary(self, preset: str, negative: str) -> dict[str, Any]:
        files = []
        ...
        return {"positive_prompt_targets": [f"{n}.{f}" for n, f in self.positive_targets],
                ...
                "primary_sampler": self.primary_sampler,
                "decode_node": self.decode_node, "model_loader": self.model_loader,
                "model_files": files, "workflow_sampler_settings": settings,
                "effective_sampling_preset": preset, "refiner_enabled": False,
...
def setup_logging(log_dir: Path, verbose: bool) -> Path:
    log_dir.mkdir(parents=True, exist_ok=True); path = log_dir / f"pixart_sigma_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    while (pending or inflight) and not stop[0]:
        while pending and len(inflight) < args.queue_size and not stop[0]:
            ...
            row, failures = pending.popleft(); workflow = spec.render(
                row_job(row), args.run_name, row["negative_prompt"], args.sampling_preset
            )
...
def build_parser(script_dir: Path):
    ...
    p.add_argument("--prompts", type=Path, default=packaged); p.add_argument("--workflow", type=Path, default=script_dir / "input/pixart_simple.json")
    ...
    p.add_argument("--state-dir", type=Path, default=script_dir / "state/pixart_sigma"); p.add_argument("--log-dir", type=Path, default=script_dir / "logs")
    ...
    p.add_argument("--http-timeout", type=float, default=30); p.add_argument("--job-timeout", type=float, default=1800); p.add_argument("--seed-salt", default="t2i-pixart-sigma-v1")
    p.add_argument("--run-name"); p.add_argument("--client-id", default="vast-pixart-sigma-batch-generator"); p.add_argument("--size-mode", choices=("prompt","workflow"), default="prompt")
...
def main(argv=None):
    ...
    try:
        ...
        unique_sizes = sorted({j.size_key for j in jobs}); plan = {"model": MODEL, "prompt_count": len(prompts), "total_jobs": len(jobs),
            ...
            "run_name": args.run_name, "workflow": spec.summary(args.sampling_preset, args.negative_prompt)}
    ...
    try:
        ...
        workflow_digest = hashlib.sha256(json.dumps(spec.template, sort_keys=True).encode()).hexdigest()
        generation_config = json.dumps({"model": MODEL, "sampling_preset": args.sampling_preset,
                                        "workflow_sha256": workflow_digest, "run_name": args.run_name}, sort_keys=True)
